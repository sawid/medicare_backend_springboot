    ...
    public ResponseEntity<?> createNewSchedule(@RequestBody AddSchedule addSchedule) {
        try{
            ...
            if(!employee.isPresent()){
                return ResponseEntity.status(400).body("Employee with ID : " + addSchedule.getEmployeeId() + " not exits");
            }

            // check is employee busy
            boolean isBusy = false;
            //get every ScheduleId of employee
            List<TakeSchedule> takeSchedules = takeScheduleService.getTakeScheduleByEmployeeId(addSchedule.getEmployeeId());
            for(TakeSchedule takeSchedule : takeSchedules){
                //check is busy
                isBusy = scheduleService.isBusy(addSchedule.getScheduleStart(), addSchedule.getScheduleEnd(), takeSchedule.getScheduleId());
                if(isBusy){
            ...
            String data = takeScheduleService.createNewTakeSchedule(schedule.getScheduleId(), addSchedule.getEmployeeId());
    ...
    public ResponseEntity<?> updateSchedule(@RequestBody UpdateSchedule updateSchedule){
        ...
        try{
            ...
            if(!schedule.isPresent()){
                return ResponseEntity.status(400).body("Schedule with ID : " + updateSchedule.getScheduleId() + " dose not exist");
            }
            ...
            if(updateSchedule.getAppointmentDoctorId() != 0){
                ...
                if(!employee.isPresent()){
                    return ResponseEntity.status(400).body("Employee with ID : " + updateSchedule.getAppointmentDoctorId() + " not exits");
             }
            }
            ...
            if(updateSchedule.getScheduleCapacity() != 0){
                List<Appointment> appointments = appointmentService.getAppointmentByScheduleId(updateSchedule.getScheduleId());
            ...
            if(updateSchedule.getScheduleDate() != null || updateSchedule.getScheduleStart() != null || updateSchedule.getScheduleEnd() != null || updateSchedule.getAppointmentDoctorId() != 0){
                List<TakeSchedule> takeSchedules = new ArrayList<>();
                //if update employeeId -> check new employeeId
                if(updateSchedule.getAppointmentDoctorId() != 0){
                    takeSchedules = takeScheduleService.getTakeScheduleByEmployeeId(updateSchedule.getAppointmentDoctorId());
                } else {
                //if not update employeeId -> check old employeeId
                    Optional<TakeSchedule> takeSchedule = takeScheduleService.getTakeScheduleByScheduleId(updateSchedule.getScheduleId());
            ...
            if(updateSchedule.getScheduleDate() != null || updateSchedule.getScheduleStart() != null || updateSchedule.getScheduleEnd() != null){
                //get list of patients of schedule
                List<Appointment> appointments = appointmentService.getAppointmentByScheduleId(updateSchedule.getScheduleId());
                for(Appointment appointment : appointments){
                    //check is patient busy
                    isBusy = appointmentService.isPatientBusy(start, end, appointment.getAppointmentPatientId(), updateSchedule.getScheduleId());
                    if(isBusy){
            ...
            if(updateSchedule.getScheduleDate() != null || updateSchedule.getScheduleStart() != null || updateSchedule.getScheduleEnd() != null || updateSchedule.getScheduleLocation() != null){
            boolean isAvaliable = scheduleService.isAvaliable( location, date, start, end, updateSchedule.getScheduleId());
            if (!isAvaliable) {
                return ResponseEntity.status(400).body("Location is taken at that time");
            }
            }
            ...
            if(patientHNId.isEmpty()){ // if list of patientHNId of edited appointment is empty
                return ResponseEntity.ok().body("Schedule with ID: " + updateSchedule.getScheduleId() + " has update successfully");
            }
